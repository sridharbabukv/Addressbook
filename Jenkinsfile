...
stages 
{
        ...
        stage('Remove Container on QA env')
        {
            steps {    
                script {
                  FILENAME = bat(label: 'Get file name', returnStdout: true, script:"@docker ps -q --filter  name=QAServer").trim()
        ...
        stage('Build Docker Image for QA env')
        {
            steps {
        ...
        stage('Push Docker Image to QA env'){
            steps {
